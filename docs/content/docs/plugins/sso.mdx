...
* `client_secret_basic` — HTTP Basic Auth header (default)
* `client_secret_post` — credentials in the request body
* `private_key_jwt` — signed JWT assertion ([RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523))

Discovery automatically selects the best supported method from the IdP's metadata. If the IdP only advertises `private_key_jwt`, it will be selected — but you must also configure a `resolvePrivateKey` callback (see below).

<Callout type="info">
If your IdP advertises only unsupported methods (e.g., `tls_client_auth` or `"none"` for public clients), you can explicitly override the method:
...
For IdPs that require `private_key_jwt` (e.g., Okta, GOV.UK One Login, Azure AD), configure the provider with `tokenEndpointAuthentication: "private_key_jwt"` and provide a `resolvePrivateKey` callback in the SSO plugin options. The `clientSecret` field is optional for `private_key_jwt` providers.
...
const auth = betterAuth({
    plugins: [
        sso({
            resolvePrivateKey: async ({ providerId, keyId, issuer }) => {
                // Load the private key from your secret store (Vault, KMS, env, etc.)
                const key = await mySecretStore.get(`sso/${providerId}/private-key`);
                return {
                    privateKeyJwk: key.jwk,
                    algorithm: "RS256",
                };
            },
        }),
...
When registering the provider, set the auth method and optionally a key ID:

```ts
await authClient.sso.registerProvider({
    issuer: "https://idp.example.com",
    domain: "example.com",
    oidcConfig: {
        clientId: "your-client-id",
        tokenEndpointAuthentication: "private_key_jwt",
        privateKeyId: "my-signing-key",
...
The `resolvePrivateKey` callback receives `providerId`, `keyId`, and `issuer`, and must return the private key material in JWK or PEM format. This keeps private keys out of the database and supports HSM/KMS integration.

For static test configurations, you can also provide inline keys via `defaultSSO`:
    ...
    defaultSSO: [
        {
            ...
            oidcConfig: {
                ...
                pkce: true,
                tokenEndpointAuthentication: "private_key_jwt",
                discoveryEndpoint: "https://test-idp.example.com/.well-known/openid-configuration",
            },
            privateKey: {
...
* `private_key_jwt` is supported via the `resolvePrivateKey` callback or `defaultSSO` inline keys
